...
class VESCController(Node):
    ...
    def destroy_node(self):
        # Clean up resources or add custom logic here
...
def main(args=None):
    ...
    node.add_motor(motor_id=111, control_type='current', topic='pump', max_value=1.0)
    ...
    node.add_motor(motor_id=36, control_type='duty_cycle', topic='linear1', max_value=0.10)
    node.add_motor(motor_id=64, control_type='duty_cycle', topic='linear2', max_value=0.10)
    node.add_motor(motor_id=47, control_type='duty_cycle', topic='mixer', max_value=1.0)
